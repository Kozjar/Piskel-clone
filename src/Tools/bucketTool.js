...
function compareDataColors(data, point, color) {
  if (data[point + 0] === color.r
    ...
    && data[point + 2] === color.b
    && data[point + 3] === color.a) return true;
  return false;
}
...
function mouseDown() {
  ...
  const pixelData = this.context.getImageData(this.state.mouse.x, this.state.mouse.y, 1, 1).data;
  const color = {
  ...
  while (stack.length > 0) {
    pixel = stack.pop();
    if (pixel[0] < 0 || pixel[0] >= this.props.canvasSize) continue;
    if (pixel[1] < 0 || pixel[1] >= this.props.canvasSize) continue;

    // Alpha
    point = pixel[1] * 4 * this.props.canvasSize + pixel[0] * 4;
